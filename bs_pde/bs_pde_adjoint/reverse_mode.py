...
def bs_pde_adjoint_b(S0: float, sigma: float, r: float, option: Function, B: np.ndarray, p: list,
                     qoi_bar: float = 1) :
...
def bs_pde_adjoint_reverse(S0: float, sigma: float, r: float, option: Function, qoi_bar: float = 1) :
    ...
    # backward pass
    S0_bar, sigma_bar, r_bar = bs_pde_adjoint_b(S0, sigma, r, option, B, p, qoi_bar)
    return S0_bar, sigma_bar, r_bar

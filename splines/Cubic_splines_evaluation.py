...
class Cubic_splines_evaluation(Black_box):
    ...
    def copy(self) :
        return Cubic_splines_construction(self.S, self.cubic_splines_construction)
    ...
    def evaluate(self, is_complex=False) -> float:
        ...
        for j in range(len(self.S)):
            p = np.searchsorted(S_p, self.S[j])
            h_p = S_p[p] - S_p[p-1]
            summand1 = sigma_p_double_prime[p-1] * (S_p[p] - self.S[j]) ** 3 / (6 * h_p)
            summand2 = sigma_p_double_prime[p] * (self.S[j] - S_p[p - 1]) ** 3 / (6 * h_p)
            summand3 = (sigma_p[p-1] - (sigma_p_double_prime[p-1] * h_p**2)/6) * (S_p[p] - self.S[j]) / h_p
            summand4 = (sigma_p[p] - (sigma_p_double_prime[p] * h_p**2)/ 6) * (self.S[j] - S_p[p - 1]) / h_p
    ...
    def forward(self, diff_u):
        ...
        for j in range(len(self.S)):
            p = np.searchsorted(S_p, self.S[j])
            h_p = S_p[p] - S_p[p-1]
            diff_summand1 = diff_sigma_p_double_prime[p-1] * (S_p[p] - self.S[j]) ** 3 / (6 * h_p) \
                            - sigma_p_double_prime[p-1] * (S_p[p] - self.S[j]) ** 2 / (2 * h_p) * diff_S[j]
            diff_summand2 = diff_sigma_p_double_prime[p] * (self.S[j] - S_p[p - 1]) ** 3 / (6 * h_p) \
                            + sigma_p_double_prime[p] * (self.S[j] - S_p[p - 1]) ** 2 / (2 * h_p) * diff_S[j]
            diff_summand3 = (diff_sigma_p[p-1] - (diff_sigma_p_double_prime[p-1] * h_p**2)/6) * (S_p[p] - self.S[j]) / h_p \
                            - (sigma_p[p-1] - (sigma_p_double_prime[p-1] * h_p**2)/6) * diff_S[j] / h_p
            diff_summand4 = (diff_sigma_p[p] - (diff_sigma_p_double_prime[p] * h_p**2)/ 6) * (self.S[j] - S_p[p - 1]) / h_p \
    ...
    def reverse(self, sigma_bar: np.ndarray):
        ...
        for j in range(len(self.S)):
            p = np.searchsorted(S_p, self.S[j])
            h_p = S_p[p] - S_p[p-1]
            sigma_p_bar[p] = sigma_p_bar[p] + sigma_bar[j] * (self.S[j]-S_p[p-1]) / h_p
            sigma_p_bar[p-1] = sigma_p_bar[p-1] + sigma_bar[j] * (S_p[p] - self.S[j]) / h_p
            sigma_p_double_prime_bar[p] = sigma_p_double_prime_bar[p] + \
                sigma_bar[j] * ((self.S[j] - S_p[p-1])**3/(6*h_p) - h_p/6 * (self.S[j] - S_p[p-1]))
            sigma_p_double_prime_bar[p-1] = sigma_p_double_prime_bar[p-1] + \
                sigma_bar[j] * ((S_p[p] - self.S[j])**3/(6*h_p) - h_p/6 * (S_p[p] - self.S[j]))
            summand1 = - sigma_p_double_prime[p-1] * (S_p[p] - self.S[j]) **2 / (2*h_p)
            summand2 = sigma_p_double_prime[p] * (self.S[j] - S_p[p-1])**2 / (2*h_p)

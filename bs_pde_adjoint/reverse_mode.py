...
def bs_pde_adjoint_b(S: float, sigma: float, r: float, option: Payoff, B: np.ndarray, p_all_list: list, qoi_bar: float = 1) :
    S_bar = p_all_list[-1] * option.diff_payoff(S) * qoi_bar  # first/second multiplication is elementwise/scalar
    p_bar = option.payoff(S) * qoi_bar
...
def bs_pde_adjoint_reverse(S0: float, sigma: float, r: float, option: Payoff, qoi_bar: float = 1) :
    # forward pass
